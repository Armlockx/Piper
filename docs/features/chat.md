...
| `chat_reply_jobs` | async LLM reply queue |
...
`lib/groq/buildChatPrompt.ts` — persona + house style + compiled traits + mood + summary + last ~40 messages.
Job type `chat`. Mood JSON update every few turns (`mood`).
